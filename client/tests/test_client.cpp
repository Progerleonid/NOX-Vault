...
TEST_CASE("secret serialization roundtrips and rejects unknown formats") {
    EncryptedValue value{1,crypto_algorithm,Bytes{1,2,3},Bytes(24,4)}; auto json=serialize_secret("",std::string("github"),std::nullopt,value);
...
TEST_CASE("encrypted private name format roundtrips and binds to record") {
    CryptoService crypto; auto key=crypto.random_vault_key(); Bytes name{'g','i','t','h','u','b'};
    ...
    auto packed=pack_encrypted_name(encrypted);packed[0]^=1;
...
TEST_CASE("local agent rejects malformed keys and enforces absolute timeout") {
    ...
    for (int i = 0; i < 50 && !client.available(); ++i) {
...
TEST_CASE("password rewrap preserves the vault key and secret ciphertext") {
    CryptoService crypto;auto p1=crypto.default_kdf();auto p2=crypto.default_kdf();auto key=crypto.random_vault_key();auto old_kek=crypto.derive_kek("old",p1);auto wrapped=crypto.encrypt(key,old_kek,"vault");
    Bytes plain{'s'};auto secret=crypto.encrypt(plain,key,"secret");auto unwrapped=crypto.decrypt(wrapped,old_kek,"vault");auto new_kek=crypto.derive_kek("new",p2);auto rewrapped=crypto.encrypt(unwrapped,new_kek,"vault");auto final_key=crypto.decrypt(rewrapped,new_kek,"vault");
    REQUIRE(final_key==key);REQUIRE(crypto.decrypt(secret,final_key,"secret")==plain);
}
TEST_CASE("local agent supports cross-client unlock timeout and explicit lock") {
    ...
    for (int i = 0; i < 50 && !first.available(); ++i) {
        std::this_thread::sleep_for(std::chrono::milliseconds(20));
    }
