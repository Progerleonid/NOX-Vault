...
CryptoService::CryptoService() {
    if (sodium_init() < 0)
        throw CryptoError("Unable to initialize libsodium");
}
KdfParameters CryptoService::default_kdf() const {
    KdfParameters result;
    result.salt.resize(crypto_pwhash_SALTBYTES);
    randombytes_buf(result.salt.data(), result.salt.size());
    result.ops_limit = crypto_pwhash_OPSLIMIT_INTERACTIVE;
    result.mem_limit = crypto_pwhash_MEMLIMIT_INTERACTIVE;
    return result;
}
Bytes CryptoService::random_vault_key() const {
...
Bytes CryptoService::derive_kek(const std::string &password, const KdfParameters &p) const {
    if (p.algorithm != "argon2id" || p.salt.size() != crypto_pwhash_SALTBYTES ||
        p.ops_limit > std::numeric_limits<unsigned long long>::max())
        throw CryptoError("Invalid vault KDF parameters");
    Bytes key(crypto_aead_xchacha20poly1305_ietf_KEYBYTES);
    if (crypto_pwhash(key.data(), key.size(), password.data(), static_cast<unsigned long long>(password.size()),
                      p.salt.data(), static_cast<unsigned long long>(p.ops_limit), p.mem_limit,
...
EncryptedValue CryptoService::encrypt(const Bytes &plaintext, const Bytes &key, const std::string &aad) const {
    if (key.size() != crypto_aead_xchacha20poly1305_ietf_KEYBYTES)
        throw CryptoError("Invalid encryption key");
    EncryptedValue out;
    ...
    out.ciphertext.resize(static_cast<std::size_t>(size));
    return out;
}
Bytes CryptoService::decrypt(const EncryptedValue &value, const Bytes &key, const std::string &aad) const {
    if (value.version != crypto_format_version || value.algorithm != crypto_algorithm ||
        ...
        value.nonce.size() != crypto_aead_xchacha20poly1305_ietf_NPUBBYTES ||
        value.ciphertext.size() < crypto_aead_xchacha20poly1305_ietf_ABYTES)
        throw CryptoError("Invalid encrypted value");
    Bytes out(value.ciphertext.size() - crypto_aead_xchacha20poly1305_ietf_ABYTES);
    unsigned long long size = 0;
    if (crypto_aead_xchacha20poly1305_ietf_decrypt(
            out.data(), &size, nullptr, value.ciphertext.data(), value.ciphertext.size(),
...
std::string CryptoService::vault_key_aad(const std::string &user_id) {
    return "nox:v1:vault-key:" + user_id;
}
std::string CryptoService::secret_aad(const std::string &vault_id, const std::string &name) {
    return "nox:v1:secret:" + vault_id + ":" + name;
}
std::string CryptoService::private_secret_aad(const std::string &vault_id, const std::string &id) {
    return "nox:v1:private-secret:" + vault_id + ":" + id;
}
std::string CryptoService::private_name_aad(const std::string &vault_id, const std::string &id) {
    return "nox:v1:private-name:" + vault_id + ":" + id;
}
...
void CryptoService::wipe(std::string &value) noexcept {
    if (!value.empty())
        sodium_memzero(value.data(), value.size());
    value.clear();
}
