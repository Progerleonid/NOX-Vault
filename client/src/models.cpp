...
SecretRecord parse_secret(const nlohmann::json& j) {
    try {
        SecretRecord value;
        value.id = j.at("id").get<std::string>();
        if (j.contains("name") && !j.at("name").is_null()) value.name = j.at("name").get<std::string>();
        if (j.contains("encrypted_name") && !j.at("encrypted_name").is_null()) value.encrypted_name = unpack_encrypted_name(base64_decode(j.at("encrypted_name").get<std::string>()));
        if (value.name.has_value() == value.encrypted_name.has_value()) throw CryptoError("Invalid secret metadata mode");
...
nlohmann::json serialize_vault_create(const EncryptedValue& key, const KdfParameters& kdf, bool private_metadata) {
    ...
    return {{"encrypted_vault_key", base64_encode(key.ciphertext)}, {"vault_key_nonce", base64_encode(key.nonce)},
        ...
        {"kdf_ops_limit", kdf.ops_limit}, {"kdf_mem_limit", kdf.mem_limit}, {"private_metadata", private_metadata}};
}
nlohmann::json serialize_secret(const std::string& id, const std::optional<std::string>& name,
    ...
    nlohmann::json result={{"ciphertext", base64_encode(value.ciphertext)}, {"nonce", base64_encode(value.nonce)},
        {"algorithm", value.algorithm}, {"version", value.version}};
    if (!id.empty()) result["id"]=id;
    if (name) { result["name"]=*name; result["encrypted_name"]=nullptr; }
    else if (encrypted_name) { result["name"]=nullptr; result["encrypted_name"]=base64_encode(pack_encrypted_name(*encrypted_name)); }
    else throw CryptoError("Secret name is missing");
    return result;
}

std::string random_uuid() {
    std::array<unsigned char,16> b{}; randombytes_buf(b.data(),b.size()); b[6]=(b[6]&0x0f)|0x40; b[8]=(b[8]&0x3f)|0x80;
    static constexpr char hex[]="0123456789abcdef"; std::string out; out.reserve(36);
    for(std::size_t i=0;i<b.size();++i){ if(i==4||i==6||i==8||i==10) out.push_back('-'); out.push_back(hex[b[i]>>4]); out.push_back(hex[b[i]&15]); }
    return out;
}
Bytes pack_encrypted_name(const EncryptedValue& value) {
    validate_format(value.version,value.algorithm); if(value.nonce.size()!=24) throw CryptoError("Invalid encrypted name nonce");
    static constexpr unsigned char magic[]={'N','X','N','M',1}; Bytes out(std::begin(magic),std::end(magic));
    out.insert(out.end(),value.nonce.begin(),value.nonce.end()); out.insert(out.end(),value.ciphertext.begin(),value.ciphertext.end()); return out;
}
EncryptedValue unpack_encrypted_name(const Bytes& value) {
    if(value.size()<5+24+16 || value[0]!='N'||value[1]!='X'||value[2]!='N'||value[3]!='M'||value[4]!=1) throw CryptoError("Invalid encrypted name format");
